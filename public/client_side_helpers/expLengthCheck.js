document.addEventListener('DOMContentLoaded', function () {
  ...
  experience.onkeyup = function() {
    if (experience.value.length < 501) {
      if (experience.classList.contains('form-control-warning') && experienceFormGroup.classList.contains('has-warning')) {
        experience.classList.remove('form-control-warning')
        experienceFormGroup.classList.remove('has-warning')
      }
      if (!experience.classList.contains('form-control-success') && !experienceFormGroup.classList.contains('has-success')) {
